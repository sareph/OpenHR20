 ...
 * \date       $Date: 2011-03-06 00:44:56 +0100 (N, 06 mar 2011) $
 * $Rev: 319 $
 */
...
static int16_t EEPROM_write(uint16_t address, uint8_t data)
{
	/* Wait for completion of previous write */
...
{
	int16_t address = dow * RTC_TIMERS_PER_DOW + slot;
	return eeprom_timers_write_raw(address, value);
}

 ...
 * \date       $Date: 2010-02-02 23:29:38 +0100 (Wt, 02 lut 2010) $
 * $Rev: 278 $
 */
 ...
 *
 ******************************************************************************/
int16_t RTC_DowTimerSet(rtc_dow_t dow, uint8_t slot, uint16_t time, timermode_t timermode)
{
	if (dow >= _DOW_MAX)
	{
		return -1;
	...
	return eeprom_timers_write(dow, slot, time | ((uint16_t)timermode << 12));
}

 ...
 * \date       $Date: 2013-01-17 21:52:23 +0100 (Cz, 17 sty 2013) $
 * $Rev: 367 $
 */
...
const static uint8_t Km_upper[8] PROGMEM = {
	0x01, 0x23, 0x45, 0x67, 0x89, 0xab, 0xcd, 0xef
};

const static uint8_t wl_header[4] PROGMEM = {
	0xaa, 0xaa, 0x2d, 0xd4
};
...
#else
					if (mac_ok && (rfm_framebuf[1] == 0))
					{ // Accept commands from master only
						wireless_buf_ptr = 0;
						RTC_timer_destroy(WL_TIMER_RX_TMO);
						if (rfm_framepos == 4 + 2)

 ...
 * \date       $Date: 2012-02-17 17:19:57 +0100 (Pt, 17 lut 2012) $
 * $Rev: 365 $
 */
...
{
	if (c != '\0')
	{  // ascii based protocol, \0 char is not alloweed, ignore it
		...
		switch (c = COM_getchar())
		{
			...
			case 'W':
			{
				...
				else
				{
					...
					int dow, slot, tim, mode;

					dow = com_hex[0] >> 4;
					slot = com_hex[0] & 0xf;
					tim = (((uint16_t)(com_hex[1]) & 0xf) << 8) + (uint16_t)(com_hex[2]);
					mode = (com_hex[1]) >> 4;

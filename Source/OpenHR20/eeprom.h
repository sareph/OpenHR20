 ...
 * \date       $Date: 2011-04-04 18:30:53 +0200 (Pn, 04 kwi 2011) $
 * $Rev: 358 $
 */
...
typedef struct
{ // each variables must be uint8_t or int8_t without exception
...
#define BOOT_ON1       ((( 7 * 60) + 50) | 0x2000)
#define BOOT_OFF1      (((21 * 60) +  0) | 0x1000)
...
uint16_t EEPROM ee_timers[8][RTC_TIMERS_PER_DOW] = { //128bytes 
	 ...
	 */
	{BOOT_ON1, BOOT_OFF1, 0x2FFF, 0x1FFF, 0x2FFF, 0x1FFF, 0x2FFF, 0x1FFF},
	{BOOT_ON1, BOOT_OFF1, 0x2FFF, 0x1FFF, 0x2FFF, 0x1FFF, 0x2FFF, 0x1FFF},
	{BOOT_ON1, BOOT_OFF1, 0x2FFF, 0x1FFF, 0x2FFF, 0x1FFF, 0x2FFF, 0x1FFF},
	{BOOT_ON1, BOOT_OFF1, 0x2FFF, 0x1FFF, 0x2FFF, 0x1FFF, 0x2FFF, 0x1FFF},
	{BOOT_ON1, BOOT_OFF1, 0x2FFF, 0x1FFF, 0x2FFF, 0x1FFF, 0x2FFF, 0x1FFF},
...
uint8_t EEPROM ee_config[][4] = {  // must be alligned to 4 bytes
	...
	/* 23 */{110, 110, 80, 160},   //!< bat_warning_thld; treshold for battery warning [unit 0.02V]=[unit 0.01V per cell]
	/* 24 */{90, 90, 80, 160},   //!< bat_low_thld; treshold for battery low [unit 0.02V]=[unit 0.01V per cell]
...
int16_t eeprom_timers_write(int16_t dow, int16_t slot, uint16_t value);
int16_t eeprom_timers_write_raw(int16_t offset, uint16_t value);
